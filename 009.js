...
console.log(data.filter((value) => new Date(value.created_at).getMonth() == 1));

console.log("Grand total all purchases: ",
    data
    .filter((value) => value.customer.name == "Ari")
    .map((value) => value.items.map((item) => item.qty * item.price).reduce((x, y) => x + y))
    .reduce((x, y) => x + y)
);
console.log("Grand total purchases under 300000 : ", [
    ...new Set(
        data
        .filter((value) => value.items.map((item) => item.qty * item.price).reduce((x, y) => x + y) < 300000)
